    ...
    public class InputBindingUGUI : MonoBehaviour
    {
       ...
       public InputBindingForInputSystem InputBinding;
        ...
        public virtual void UpdateDisplayName()
        {
            ...
            string path = InputBinding.GetBindingPath();
            if (path.IndexOf(InputBindingForInputSystem.CompositeControlSeparator) >= 0)
            {
                ...
                string localizedSeparator = localizeString("CompositeControlSeparator");
                ...
                {
                    if (i == 0)
                        localizedDisplayName = localizeString(paths[i]);
                    else
                        localizedDisplayName += localizedSeparator + localizeString(paths[i]);
                }
                DisplayName = localizedDisplayName;
            }
            else
            {
                DisplayName = localizeString(InputBinding.GetBindingPath());
            }
        }
        ...
        private string localizeString(string path)
        {
            if (PathToDisplayNameFunc != null)
                return PathToDisplayNameFunc(path);
            else
                return path;
        }
