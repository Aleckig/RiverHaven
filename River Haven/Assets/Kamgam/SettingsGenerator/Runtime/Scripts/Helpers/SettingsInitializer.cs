...
namespace Kamgam.SettingsGenerator
{
    ...
    [DefaultExecutionOrder(-10)] // Not strictly necessary. We use it to make the Initializer
                                 // run before any of the other settings code. NOTICE: This is bugged
                                 // on some platforms and Unity versions, see:
                                 // https://discussions.unity.com/t/script-execution-order-settings-seem-to-be-ignored/927131/17
    public class SettingsInitializer : MonoBehaviour
    {
        ...
        public void Awake()
        {
            ...
            //
            // If for some reason you must initialized the settings in Awake() then please use 
...
#if UNITY_EDITOR
        [CustomEditor(typeof(SettingsInitializer))]
        public class SettingsInitializerEditor : Editor
        {
            public override void OnInspectorGUI()
            {
                EditorGUILayout.HelpBox("The SettingsInitializer is not longer needed. Please consider removing it and use the configuration in the SettingsProvider instead." +
                                        "\n\nHowever, if a SettingsInitializer is found in any scene it will be used and the provider init configuration will be ignored. This is done to remain backwards compatible with older versions but it's no longer recommended to use the initializer.", MessageType.Warning);
